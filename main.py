...
def scan_chromosome(name, samfile, bin_length, chromosome_length, ref_sequence):
    start = 0
    ...
    while start < chromosome_length:
        reads = samfile.fetch(name, start=start, stop=start + bin_length)
        i = 0
        # total_bases = 0
        fragment = ref_sequence[start:(start+bin_length)]
        gc_percent = GC(fragment)
        ...
        counts.append(i)
        starts.append(start)
        gc_percents.append(int(gc_percent))
        prt += 1
        if prt == 1000:
...
def call_variants(segments, rd_corrected):
    ...
    for i in range(1, len(segments)):
        ...
        n = end - start
        if np.abs(rd_global - rd_segment) < rd_global / 4:
            continue
        if s_segment == 0:
            calls.append((start, end - 1))
            is_deletion.append((rd_global - rd_segment) > 0)
            continue
...
def merge_calls(calls, rd_corrected, is_deletion):
    ...
    new_is_deletion.append(is_deletion[-1])
...
def definitely_not_cnvnator(path, chromosomes, bin_length):
    ...
    for chromosome in chromosomes:
        ref_sequence = reference[chromosome].seq
        ...
        rd_corrected, rd_global = gc_correction(counts_trimmed, gc_percents[trim_length:len(gc_percents)])
        print(np.unique(rd_corrected))
        segments = mean_shift(counts_trimmed, rd_corrected, rd_global)
        ...
        variant_calls.extend(
            create_variant_calls(reference, merged_calls, bin_length, chromosome, trim_length, is_deletion_merged))
        plt.plot(rd_corrected)
        for i, call in enumerate(merged_calls):
            if is_deletion[i]:
                plt.axvspan(call[0], call[1], facecolor='red', alpha=.2)
            else:
                plt.axvspan(call[0], call[1], facecolor='green', alpha=.2)
